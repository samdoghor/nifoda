...
>(({text = "Button", className, ...props}, ref) => {
    return (
        <button
            ref={ref}
            className={cn(
                "group relative w-52 cursor-pointer overflow-hidden rounded-full border p-2 text-center font-semibold",
                className,
            )}
            {...props}
        >
      <span
        ...
        {text}
            ...
            <div
                className="absolute left-[20%] top-[40%] h-2 w-2 scale-[1] rounded-lg bg-primary transition-all duration-300 group-hover:left-[0%] group-hover:top-[0%] group-hover:h-full group-hover:w-full group-hover:scale-[1.8] group-hover:bg-black"></div>
        </button>
    );
});
